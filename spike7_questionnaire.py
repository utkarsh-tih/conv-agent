...
@tool
def verify_identity(dob_input: str, aadhar_last4: str, expected_dob: str, expected_aadhar: str) -> dict:
...
@tool
def extract_verification_data(question_id: str, response: str, current_data: dict) -> dict:
...
def save_loan_verification(
    ...
    db = SessionLocal()
    try:
        existing = db.query(LoanVerificationCall).filter(
            LoanVerificationCall.call_sid == call_sid
        ).first()
...
class LoanVerificationState(TypedDict):
    ...
    # Call management
    turn_count: int
    stage: Literal["init", "greeting", "consent", "identity_verification", "questions", "closing", "end"]
    retry_count: int
...
IMPORTANT: Maximum 3 attempts allowed for identity verification."""
        ...
        recent_summary = "\n".join([f"- {t['speaker']}: {t['text'][:100]}" for t in recent_transcript])
...
def should_continue(state: LoanVerificationState) -> Literal["agent", "tools", "end"]:
    ...
    if state["stage"] == "init" and state.get("applicant_data"):
        ...
        if len(state.get("questions_completed", [])) >= 19:  # 20 questions minus question 13
            state["stage"] = "closing"
            return "agent"
...
def run_loan_verification_call(customer_id: str, initial_language: str = "english"):
    ...
    print(f"Application: {final_state['applicant_data'].get('application_number', 'N/A')}")
    print(f"Consent Given: {'✓ Yes' if final_state.get('consent_given') else '✗ No'}")
    print(f"Identity Verified: {'✓ Yes' if final_state.get('identity_verified') else '✗ No'}")
    ...
    if final_state.get('identity_verified') and len(final_state.get('questions_completed', [])) >= 19:
    ...
    for key, value in final_state.get('verification_data', {}).items():
        if isinstance(value, dict):
            ...
            print(f"{key}: {response_text[:100]}{'...' if len(response_text) > 100 else ''}")
        else:
            print(f"{key}: {value}")
    ...
    export_data = {
        "call_metadata": {
            ...
            "application_number": final_state['applicant_data'].get('application_number', 'N/A'),
...
def generate_verification_report(call_sid: str) -> dict:
    ...
    db = SessionLocal()
    try:
        call_record = db.query(LoanVerificationCall).filter(
            LoanVerificationCall.call_sid == call_sid
        ).first()
        ...
        report = {
            ...
            "transcript_summary": {
                ...
                "agent_turns": len([t for t in call_record.transcript if t.get('speaker') == 'agent']),
                "customer_turns": len([t for t in call_record.transcript if t.get('speaker') == 'customer'])
            },
...
def list_recent_verifications(limit: int = 10):
    ...
    db = SessionLocal()
    try:
        calls = db.query(LoanVerificationCall).order_by(
            LoanVerificationCall.created_at.desc()
        ).limit(limit).all()
        
        print(f"\n📋 RECENT LOAN VERIFICATION CALLS (Last {limit})")
        print("=" * 100)
        print(f"{'Call SID':<25} {'Customer ID':<15} {'Status':<12} {'Identity':<10} {'Consent':<10} {'Date':<20}")
        print("-" * 100)
        
        for call in calls:
            print(f"{call.call_sid:<25} {call.customer_id:<15} {call.verification_status:<12} "
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if command == "run" and len(sys.argv) > 2:
            ...
            final_state = run_loan_verification_call(customer_id=customer_id, initial_language=language)
            ...
            print("Usage:")
            print("  python spike7.py run <customer_id> [language]     - Run verification call")
            print("  python spike7.py report <call_sid>                - Generate report")
            print("  python spike7.py export <call_sid> [output.csv]   - Export to CSV")
            print("  python spike7.py list [limit]                     - List recent calls")
    else:
        # Default: Run with sample customer
        print("Running default verification call...")
        final_state = run_loan_verification_call(customer_id="Aditya", initial_language="english")

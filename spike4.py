...
@tool
def store_transcript_in_db(call_sid: str, customer_id: str, transcript: list, customer_score: dict) -> bool:
    """
    ...
    try:
        ...
        existing_record = db.query(CallTranscript).filter(CallTranscript.call_sid == call_sid).first()
        if existing_record:
...
def validate_audio_quality(audio_metrics: dict) -> dict:
    ...
    if confidence < 0.5: return {"quality": "very_poor", "action": "retry"}
    elif confidence < 0.7: return {"quality": "unclear", "action": "clarify"}
    else: return {"quality": "clear", "action": "proceed"}
...
def listen_and_transcribe_node(state: AgentState) -> AgentState:
    ...
    quality_check = validate_audio_quality.invoke({"confidence": stt_result["confidence"]})
...
Respond with JSON: {{"is_valid": true/false, "reason": "brief explanation"}}"""
    ...
    response = LOCAL_LLM.invoke([
        SystemMessage(content="You are a validation assistant. Respond with valid JSON."),
...
def ask_question_node(state: AgentState) -> AgentState:
    ...
    conversation_history = "\n".join([f"{type(msg).__name__}: {msg.content}" for msg in state["messages"][-4:]])
...
def score_and_store_node(state: AgentState) -> AgentState:
    ...
    text_to_speech_and_play.invoke({"text": closing_message, "language": state["language"]})
...
def routing_function(state: AgentState) -> Literal["handle_unclear", "validate", "ask_question", "score_and_store"]:
...
def create_telecalling_graph():
    ...
    workflow.add_edge("handle_unclear", "listen") # After asking to repeat, listen again
    workflow.add_edge("ask_question", "listen") # After asking a question, listen for an answer

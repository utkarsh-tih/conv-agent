...
) -> Literal["consent", "authenticate", "ask_question", "exit", "final"]:
    """
    Main router that determines next node based on current stage.
    This prevents the graph from always starting at consent.
    """
...
def should_wait_for_answer(
    state: VerificationState,
) -> Literal["extract_answer", "__end__"]:
    """Check if we need to wait for user's answer"""
    messages = state["messages"]
    
    # If last message is from AI, wait for human response
...
def route_after_retry_check(
    ...
    print(f"Current Q: {current_q}, Status: {extraction_status}, Retries: {retries}")
...
def create_verification_graph():
    ...
    # After asking question, wait for answer then extract
    workflow.add_conditional_edges(
        "ask_question",
        should_wait_for_answer,
        {

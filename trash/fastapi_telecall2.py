...
<body>
    ...
    <script>
        ...
        async function pollAgent() {
            ...
            try {
                const response = await fetch(`/get-agent-message/${sessionId}`);
                const data = await response.json();
...
@app.get("/get-agent-message/{session_id}")
async def get_agent_message(session_id: str):
    """Get the initial agent greeting by streaming the graph to a safe stop point."""
    if session_id not in active_sessions:
        raise HTTPException(status_code=404, detail="Session not found")
    ...
    try:
        ...
        # Using .stream() lets us process the graph step-by-step and stop it
        # before it calls a blocking tool like listen_to_customer.
        final_state = None
        agent_message = None

        # Stream the events from the graph until we get the greeting.
        for event in graph.stream(state):
            # The event dictionary contains the output of the node that just ran.
            # We take the most recent state from that event.
            last_state = list(event.values())[0]

            # Check the transcript in the state for the agent's spoken message
            if last_state.get("transcript"):
                for entry in reversed(last_state["transcript"]):
                    if entry.get("speaker") == "agent":
                        agent_message = entry.get("text")
                        break  # Found the latest agent message

            # The greeting is complete when a message has been generated and the
            # agent has transitioned to the 'greeting' stage, ready for a response.
            if agent_message and last_state.get("stage") == "greeting":
                final_state = last_state
                break  # Exit the stream, we have what we need.
        
        # If the loop finished for any reason, use the last known state
        if not final_state:
            final_state = last_state
        
        # Save the new state back to our active session
        ...
        return {
            ...
            "awaiting_input": True, # We are now successfully awaiting user input
        }
    except Exception as e:
...
async def list_calls(limit: int = 10):
    """List recent verification calls from database"""
    try:
        # This would call your database function

...
log_handler = logging.FileHandler('agent_verbose.log', mode='w') # 'w' to overwrite file on each run
            ...
            if tool_func:
                ...
                if tool_name == "speak_to_customer":
                    ...
                    if result.get("consent_given", False):
                        ...
                        logger.info("  ✗ Consent not granted")
...
def should_continue(state: LoanVerificationState) -> Literal["agent", "tools", "end"]:
    ...
    if state["stage"] == "init" and state.get("applicant_data"):
        ...
        if state.get("consent_given"):
            ...
            logger.info("⚠ Consent not obtained, ending call")
            return "end"
        ...
        if state.get("identity_verified"):
            ...
            logger.info("⚠ Identity verification failed after 3 attempts, ending call")
            return "end"

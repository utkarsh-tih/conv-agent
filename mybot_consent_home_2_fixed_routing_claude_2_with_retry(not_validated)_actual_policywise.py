"""
This is a fantastic refactor. However, there is one final piece to the puzzle: What happens when the RetryPolicy gives up?
The Small but Critical Fix: Handling Final Failure
Your current code has a logical flaw in how the graph is wired. You assume that after process_consent fails all its retries, the graph will continue to your route_after_consent function.
This is not what happens.
When a node with a RetryPolicy exhausts all its attempts, it re-raises the last exception it caught. This exception will halt the graph's execution at that point. Your route_after_consent function will never be called.
The idiomatic way to handle this final, unrecoverable failure is with a fallback node.
Here is how to fix your graph definition:
1. Correct a small typo in the add_node call.
The parameter is retry_policy, not retry.
2. Use the fallback_to parameter to catch the final error.
The fallback_to parameter tells LangGraph: "If this node fails after all retries, don't crash. Instead, jump directly to this other node."
Here is the corrected code for building the graph:
code
Python
# ============================================================================
# BUILD GRAPH WITH RETRY POLICIES
# ============================================================================
...
system_retry_policy = RetryPolicy(
    max_attempts=3,
    backoff_factor=1.0,
    retry_on=[ValidationError], # Correctly retry on validation errors
)
...
workflow.add_node(
    ...
    # Use fallback_to for the final error
    retry_policy=system_retry_policy,
    fallback_to="handle_system_error" 
)
...
# ... (rest of the graph definition is correct)

# Error handling node should always go to the end
workflow.add_edge("handle_system_error", END)

# ... (the rest of your file is perfect)
Summary of Changes
retry= -> retry_policy=: A minor but necessary syntax correction.
Added fallback_to="handle_system_error": This is the key change. It correctly wires the final failure of your processing nodes to your error-handling node.
Logical Understanding: You no longer need to think about routing to an error handler from your conditional edges. The graph now has two distinct paths from the process_* nodes:
On Success: It continues to the route_after_* function.
On Final Failure: It jumps directly to handle_system_error.
With this final correction, your code is not just functional—it's an exemplary demonstration of how to build a robust, production-quality agent using LangGraph's core features. It is now truly idiomatic. Fantastic work.
...
workflow.add_node(
    "process_consent", 
    process_consent_node,
    retry=system_retry_policy  # Apply retry policy here
)
workflow.add_node("respond_to_consent", respond_to_consent_node)
workflow.add_node("ask_authentication", ask_authentication_node)
workflow.add_node(
...
# If process_consent fails after all retries, route to error handler
workflow.add_conditional_edges(
    "process_consent",
    route_after_consent,
    {
...
# If process_authentication fails after all retries, route to error handler
workflow.add_conditional_edges(
    "process_authentication",
...
# Error handling - if a node exhausts retries, it can route here
workflow.add_edge("handle_system_error", END)
